...
class Runner:
    def __init__(self, n_clusters: int, epsilon: float, data_instance: str, n_repetitions: int, output_dir: str) -> None:
    ...
    def run(self):
        ...
        print("Running")
        alpha = 1 # np.sqrt(1/epsilon)
        k = self._n_clusters
        epsilon = self._epsilon
        n_points = int(np.power(k, 1.5) / np.power(epsilon, 2))
        coreset_size = int(k / (10 * np.power(epsilon, 2)))
        X = generate_simplex(d=n_points, alpha=alpha)
        ...
        ss = SensitivitySampling(n_clusters=self._n_clusters, coreset_size=coreset_size, output_dir=self._output_dir)
        ss.run(X)
...
def main(n_clusters: int, epsilon: float, data_instance: str, n_repetitions: int, output_dir: str):
    Runner(
